...
import type { FraudNotes } from '../../types/fraud';
import {
  getAutoBlockManual,
  getAutoTrustManual,
  setAutoBlockManual,
...
export default function CallDetailScreen({
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase
        .from('calls')
        .select(
          'id, profile_id, created_at, transcript, fraud_score, fraud_risk_level, fraud_keywords, fraud_notes, caller_number, feedback_status, caller_hash, voice_synthetic_score'
        )
        .eq('id', callId)
        .single();
  ...
  const playDisabled = recordingStatus === 'error' || recordingStatus === 'loading';  const footerDisabledSafe = callRow.feedback_status === 'marked_safe';
