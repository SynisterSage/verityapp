...
# Xcode Cloud images can still be on Ruby 2.6, which is incompatible with
# newer Bundler lines. Use plain pod install for maximum compatibility.
if ! command -v pod >/dev/null 2>&1; then
  install_ruby_gem cocoapods 1.16.2
fi

PBXPROJ_PATH="VerityProtect.xcodeproj/project.pbxproj"
if [ -f "$PBXPROJ_PATH" ]; then
  # Keep objectVersion CocoaPods-compatible on runners that still ship older xcodeproj.
  run sed -i.bak 's/objectVersion = 70;/objectVersion = 60;/' "$PBXPROJ_PATH"
fi
